...
public class TSBSPathExtractor {
  ...
  private static final String[] READINGS = {"latitude","longitude","elevation","velocity","heading","grade","fuel_consumption"};
  private static final String[] DIAGNOSTICS = {"fuel_state","current_load","status"};
  ...
  public static void main(String[] args) {
    if (args.length < 2) {
      ...
      // System.exit(1);
      args = new String[] {"mtreedata/tsbs-iot-1mdev.txt", "mtreedata/tsbs_path.txt"};
    }
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(inputFile));
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (devId != null && line.startsWith("readings,")) {
          for (String s : READINGS) {
            path = devId + "." + s;
            writer.write(path);
            writer.newLine();
          }
          pathNum += READINGS.length;
        } else if (devId != null && line.startsWith("diagnostics,")) {
          for (String s : DIAGNOSTICS) {
            path = devId + "." + s;
            writer.write(path);
            writer.newLine();
          }
          pathNum += DIAGNOSTICS.length;
      ...
      System.out.println("成功生成 " + devNum + " 个 Device ID, " + pathNum + " 个序列 ID." + outputFile);
