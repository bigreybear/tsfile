...
/**
 * A simpl(istic) implementation of the EliasFano compression technique. This class compresses array
 * of MONOTONICALLY INCREASING integers into (smaller) arrays of bytes. It permits to uncompress an
 * arbitrary element for the compressed data, without decompressing the whole array. Similarly, it
 * permits to find the index of the first element in the compressed data, greater or equal to a
...
public final class EliasFano {
  ...
  /**
   * Returns the number of lower bits required to encode each element in an array of size {@code
   * length} with maximum value {@code u}.
   *
  ...
  /**
   * Compresses {@code length} elements of {@code in}, from {@code inOffset}, into the {@code out}
   * array, from {@code outOffset}
   *
   * @param in the array to compress (MONOTONICALLY INCREASING)
   * @param inOffset starting offset
   * @param length number of elements to compress
  ...
  /**
   * Decompress {@code length} elements from {@code in}, starting at {@code inOffset}, into {@code
   * out}, starting from {@outOffset}. Each element is encoded using {@code l} lower bits.
   *
   * @param in the compressed array
   * @param inOffset starting offset
   * @param length number of elements to decompress
   * @param l number of lower bits for each element
   * @param out the uncompressed array
   * @param outOffset starting offset
  ...
  /**
   * Decompresses the idx-th element from the compressed array {@code in}, starting from {@code
   * inOffset}. The uncompressed array has size {@code length} and its elements are encoded using
   * {@code l} lower bits each.
   *
   * @param in the compressed array
   * @param inOffset starting offset
   * @param length the size of the uncompressed array
   * @param l number of lower bits
   * @param idx the index of the element to decompress
   * @return the value of the idx-th element
   */
  public static int get(
      final byte[] in, final int inOffset, final int length, final int l, final int idx) {
  ...
  /**
   * Returns the index of the first element equal or greater than {@code val} from the compressed
   * array {@code in}, starting from {@code inOffset}. The uncompressed array has size {@code
   * length} and its elements are encoded using {@code l} lower bits each.
   *
   * @param in the compressed array
   * @param inOffset starting offset
   * @param length size of the uncompressed array
   * @param l number of lower bits
   * @param val value to select
   * @return the index of the first element equal or greater than {@code val}
   */
  public static int select(
      final byte[] in, final int inOffset, final int length, final int l, final int val) {
  ...
  /**
   * Returns the number of time {@code val} occurs in the compressed array {@code in}, starting from
   * {@code inOffset}. The uncompressed array has size {@code length} and its elements are encoded
  ...
  /**
   * Returns the number of bytes required to compress an array of size {@code length} and maximum
   * value {@code u}.
   *
  ...
  public static void main(String[] args) {
    ...
    List<Integer> tl = Arrays.stream(t).boxed().collect(Collectors.toList());
    tl.sort(Integer::compareUnsigned);
    t = tl.stream().mapToInt(i -> i).toArray();
    byte[] cb = compress(t, 0, t.length);
    int l = getL(t[t.length - 1], t.length);
    int v = get(cb, 0, t.length, l, 2);
