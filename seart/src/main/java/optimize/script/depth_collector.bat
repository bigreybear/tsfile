...
set filename=output_depth.txt

for /L %%i in (0,1,3) do (
    echo ============= depth for !ds[%%i]! ========= >> %filename%
    for /L %%j in (1,1,7) do (
        java -jar seart-1.0.1-SNAPSHOT.jar !mm[%%j]! -ds !ds[%%i]! -depth >> %filename% 2>&1
        echo. >> %filename%
        echo processing %%j / 7 %CR
    )
    echo. >> %filename%
)
